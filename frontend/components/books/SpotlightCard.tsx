...
export function SpotlightCard({ book }: { book: Book }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-linear-to-r from-ink/95 via-ink/85 to-ink/30" />
      <div className="absolute inset-0 bg-linear-to-t from-ink/80 via-transparent to-transparent sm:hidden" />
      ...
      <div className="relative flex flex-col justify-center px-5 py-5 sm:px-8 sm:py-7 flex-1 min-w-0">
        <span className="inline-flex items-center gap-1.5 font-mono text-[10px] sm:text-xs uppercase tracking-[0.18em] text-vermillion mb-2 sm:mb-3">
          <svg
        ...
        <h2 className="font-display text-xl sm:text-3xl md:text-4xl text-text leading-tight line-clamp-2 mb-1">
          {book.title}
        </h2>

        {book.author && (
          <p className="text-xs sm:text-sm text-text-mute mb-2 sm:mb-3 truncate italic font-display">
            {book.author}
          </p>
        )}

        {book.description && (
          <p className="hidden sm:block text-sm text-text-mute leading-relaxed line-clamp-2 mb-4">
            {book.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-3 mt-auto pt-2 sm:pt-0">
          ...
          <span className="inline-flex items-center gap-1 font-mono text-[10px] tracking-widest uppercase text-text-faint">
            <svg
