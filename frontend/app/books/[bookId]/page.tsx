    ...
    <div className="max-w-4xl mx-auto">
      ...
      <nav className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 min-w-0">
          <Link href="/" className="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0">
            Thư viện
          </Link>
          <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
          <span className="text-gray-900 dark:text-gray-100 font-medium truncate max-w-xs">{book.title}</span>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden mb-6">
        <div className="flex gap-5 sm:gap-6 p-5 sm:p-6">
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            <div className="flex-1">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 mb-1.5 leading-tight">{book.title}</h1>
              {book.author && (
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400 dark:text-gray-500 mb-3">
                ...
                <span className="flex items-center gap-1">
                  ...
                  {book.voice.includes("HoaiMy") ? "HoaiMy (Nữ)" : "NamMinh (Nam)"}
                </span>
        ...
        {isParsing ? (
          <div className="flex items-center gap-3 mx-5 sm:mx-6 mb-5 sm:mb-6 px-4 py-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Đang xử lý file EPUB...</p>
              <p className="text-xs text-amber-600 dark:text-amber-500 mt-0.5">Sẽ sẵn sàng trong giây lát</p>
            </div>
          </div>
        ) : firstChapter ? (
          <div className="grid grid-cols-2 gap-3 mx-5 sm:mx-6 mb-5 sm:mb-6">
            <Link
              href={`/books/${bookId}/listen?chapter=${firstChapter.id}`}
            ...
            >
              ...
              <div className="min-w-0">
                <p className="font-semibold text-sm">Nghe ngay</p>
                <p className="text-[11px] text-indigo-200 mt-0.5">TTS trực tiếp</p>
              </div>
            </Link>
            <Link
              href={`/books/${bookId}/read?chapter=${firstChapter.id}`}
            ...
            >
              ...
              <div className="min-w-0">
                <p className="font-semibold text-sm">Đọc truyện</p>
                <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">Đọc văn bản</p>
              </div>
            </Link>
          </div>
        ) : null}
