  ...
  if (!book) {
    return (
      <div className="text-center py-24">
        <p className="text-gray-500 dark:text-gray-400 font-medium">Không tìm thấy truyện</p>
        <Link href="/" className="text-sm text-indigo-600 hover:text-indigo-700 mt-2 inline-block">Quay lại thư viện</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 min-w-0">
        <Link href="/" className="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shrink-0">Thư viện</Link>
        ...
        <span className="text-gray-900 dark:text-gray-100 font-medium shrink-0">Chỉnh sửa</span>
      </nav>

      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Chỉnh sửa truyện</h1>
      ...
      <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Thông tin cơ bản</h2>
        </div>
        <div className="p-5">
          <BookInfoEditor
            bookId={bookId}
            book={book}
            onSaved={() => queryClient.invalidateQueries({ queryKey: ["book", bookId] })}
          />
        </div>
      </section>
      ...
      <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Thể loại</h2>
        </div>
        <div className="p-5">
      ...
      <section className="bg-white dark:bg-gray-800 rounded-2xl border border-red-200 dark:border-red-900/50 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-red-100 dark:border-red-900/30">
          <h2 className="text-sm font-semibold text-red-600 dark:text-red-400">Vùng nguy hiểm</h2>
        </div>
        <div className="p-5 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Xóa truyện</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Xóa vĩnh viễn toàn bộ truyện, chương, file audio và ảnh bìa. Không thể hoàn tác.
            </p>
          </div>
          <button
...
function BookInfoEditor({
  ...
  bookId: string;
  book: { title: string; author?: string | null; description?: string | null; cover_url?: string | null };
  onSaved: () => void;
}) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      await api.updateBook(bookId, { title, author: author || undefined, description: description || undefined, cover });
      onSaved();
      setSuccess(true);
      setCover(null);
  ...
  return (
    <form onSubmit={handleSave} className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-5">
        ...
        <div className="shrink-0 flex flex-col items-center gap-2">
          <div className="w-28 h-40 rounded-xl overflow-hidden bg-linear-to-br from-indigo-50 to-purple-50 dark:from-indigo-950 dark:to-purple-950 border border-gray-200 dark:border-gray-700 shadow-sm">
            {currentCover ? (
              <Image src={currentCover} alt="cover" width={112} height={160} className="object-cover w-full h-full" />
            ) : (
          ...
          {cover && (
            <button type="button" onClick={() => { setCover(null); setCoverPreview(null); if (fileRef.current) fileRef.current.value = ""; }}
      ...
      {/* Description */}
      <div>
        <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Mô tả / Giới thiệu</label>
        <textarea
          value={description}
      ...
      {success && <p className="text-sm text-emerald-600 dark:text-emerald-400">✓ Đã lưu thành công</p>}
...
function AddChapterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      await api.createChapter(bookId, { chapter_index: index - 1, title: title.trim(), text_content: textContent.trim() });
      onSaved();
      setSuccess(true);
      setTitle("");
      setTextContent("");
  ...
  return (
    <div>
      ...
      {open && (
        <form onSubmit={handleSubmit} className="mt-4 space-y-3 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
