...
export default function EditBookClient() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Parse-failure banner — shows WHY a book errored and points at the
          "Phân tích lại" action below so the admin can retry from the original. */}
      {book.status === "error" && (
        <div className="rounded-xl border border-vermillion/40 bg-vermillion/10 dark:bg-vermillion/15 px-4 py-3 flex gap-3">
          <svg
            className="w-5 h-5 text-vermillion shrink-0 mt-0.5"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-text-dim dark:text-text-mute mt-0.5 break-words">
              {book.error_message ||
                "Không có thông tin lỗi. Thử phân tích lại từ file gốc bên dưới."}
            </p>
          </div>
        </div>
      )}
